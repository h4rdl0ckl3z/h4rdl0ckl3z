...
const bars = [...counts.entries()].map(([date, count], index) => {
  ...
  const opacity = count === 0 ? "0.18" : "0.85";
  return `<rect x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${barWidth.toFixed(2)}" height="${barHeight.toFixed(2)}" rx="3" fill="currentColor" opacity="${opacity}"><title>${date}: ${count} public events</title></rect>`;
}).join("\n");
...
<svg xmlns="http://www.w3.org/2000/svg" width="900" height="220" viewBox="0 0 900 220" role="img" aria-labelledby="title desc">
  ...
  <g font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="10" fill="#8b949e">
    <text x="${left}" y="${height - 10}">${dates[0]}</text>
    <text x="${width - right}" y="${height - 10}" text-anchor="end">${dates.at(-1)}</text>
  </g>
</svg>
`;
