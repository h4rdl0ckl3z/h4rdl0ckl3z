# 👋 Hi, I'm Avatart0Dev

> **Reverse Engineer · Automation Developer · Full Stack Developer**
>
> I analyze how systems really work, turn that understanding into reliable automation, and build production-ready applications around it.

<p align="center">
  <a href="https://github.com/h4rdl0ckl3z">
    <img src="https://komarev.com/ghpvc/?username=h4rdl0ckl3z&label=Profile%20Views&style=flat" alt="Profile views" />
  </a>
  <a href="https://github.com/h4rdl0ckl3z?tab=followers">
    <img src="https://img.shields.io/github/followers/h4rdl0ckl3z?label=Followers&style=flat" alt="GitHub followers" />
  </a>
  <a href="https://github.com/h4rdl0ckl3z?tab=repositories">
    <img src="https://img.shields.io/github/stars/h4rdl0ckl3z?label=Stars&style=flat" alt="GitHub stars" />
  </a>
  <img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Fusers%2Fh4rdl0ckl3z&query=%24.public_repos&label=Repositories&style=flat" alt="GitHub repositories" />
</p>

## 📊 GitHub Status

<p align="center">
  <a href="https://github.com/h4rdl0ckl3z">
    <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=h4rdl0ckl3z&theme=github_dark" height="170" alt="GitHub statistics" />
  </a>
  <a href="https://github.com/h4rdl0ckl3z">
    <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=h4rdl0ckl3z&theme=github_dark" height="170" alt="Top languages" />
  </a>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=h4rdl0ckl3z&theme=github-dark&hide_border=true" alt="GitHub contribution streak" />
</p>

## 🔄 GitHub Activity

<p align="center">
  <img src="./h4rdl0ckl3z/assets/github-activity.svg" alt="GitHub activity over the last 30 days" width="900" />
</p>

> Activity data is generated from the GitHub public events API by GitHub Actions and stored in this repository, so the README does not depend on an external activity-graph service.

---

## 🧠 About Me

I'm a developer focused on **reverse engineering, automation, system integration, and full-stack development**.

I enjoy going beyond documentation to understand how software behaves in real-world conditions. I analyze requests, responses, client-side logic, validation flows, and execution paths, then use that knowledge to build practical and maintainable solutions.

My approach is simple:

**Understand → Reconstruct → Automate → Build → Improve**

I work comfortably across the stack, connecting frontend applications, backend services, APIs, automation workflows, databases, and infrastructure.

---

## 🔬 What I Do

- 🔍 Analyze system behavior and request/response flows
- 🧩 Reconstruct undocumented or complex workflows
- 🐞 Debug client-side JavaScript and validation logic
- 🤖 Build automation tools, bots, and workflow systems
- 🔗 Integrate APIs and third-party services
- 💻 Develop full-stack web applications
- ⚙️ Design backend services and REST APIs
- 🚀 Build maintainable deployment and CI/CD workflows

---

## 🛠️ Technical Stack

### 💻 Development

- **Languages:** TypeScript, JavaScript, SQL
- **Frontend:** React, Next.js, modern web tooling
- **Backend:** Node.js, REST APIs, automation services
- **Architecture:** Full Stack, API integration, event-driven systems

### 🔐 Reverse Engineering & Analysis

- Request / response analysis
- Runtime behavior analysis
- JavaScript debugging, including minified code
- Validation and token-flow inspection
- Workflow reconstruction
- API behavior analysis

### 🗄️ Data & Infrastructure

- MySQL / MariaDB
- PostgreSQL
- Docker
- CI/CD
- Linux-based development environments

---

## 🚀 Selected Work

### 🏛️ e-GP Procurement Automation

Worked with Thailand's government procurement ecosystem and its public APIs/documentation.

**Focus:**

- Analyzed actual request/response behavior
- Compared documented APIs with real execution flows
- Reconstructed multi-step workflows
- Built automation and backend integrations
- Improved reliability of repetitive system operations

### 🤖 Discord Automation

Built Node.js-based automation bots for command-driven and event-driven workflows.

**Focus:**

- Command handling
- Event-driven architecture
- Webhook integrations
- Automated workflows
- External service integration

### 🌐 Web Applications & Platforms

Build full-stack applications with an emphasis on maintainability, performance, and clean system boundaries.

```text
Frontend
   ↓
Next.js / React
   ↓
API / Backend Services
   ↓
Database / External Services
   ↓
Automation / Infrastructure
```

---

## 🎯 Current Focus

- Reverse engineering & system analysis
- Automation engineering
- Full-stack TypeScript development
- API and service integration
- Backend architecture
- Developer tooling
- Production-ready web platforms

---

## 🧭 Engineering Philosophy

> **Documentation tells you what a system should do. Runtime behavior tells you what it actually does.**

I value understanding the complete execution path instead of treating individual APIs or components in isolation.

The goal is not simply to make something work once, but to understand **why it works**, identify edge cases, and turn that knowledge into a solution that can be maintained and improved.

---

## 📫 Connect

- **GitHub:** [@h4rdl0ckl3z](https://github.com/h4rdl0ckl3z)

---

<p align="center">
  <sub>Built with curiosity, automation, and a lot of debugging.</sub>
</p>
